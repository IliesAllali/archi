...
/* ─── Compact badge (icon only, label on hover) ─── */
function CompactBadge({ type, label }: { type: string; label: string }) {
  const cfg = getConfig(type);
  ...
  if (type === "google") {
    return (
      <div className="relative group/ep">
        ...
        {/* Hover tooltip */}
        <div
          className="absolute left-1/2 -translate-x-1/2 bottom-full mb-1.5 opacity-0 group-hover/ep:opacity-100 transition-opacity duration-150 pointer-events-none whitespace-nowrap px-2 py-1 rounded-md text-[10px]"
          style={{ background: "var(--elevated)", border: "1px solid var(--line-strong)", color: "var(--text-secondary)", boxShadow: "0 2px 8px rgba(0,0,0,0.2)" }}
        >
          {label}
        </div>
      </div>
  ...
  return (
    <div className="relative group/ep">
      <div
        className="flex items-center justify-center w-[24px] h-[24px] rounded-full leading-none"
        style={{
          background: "var(--ep-bg)",
          backdropFilter: "blur(8px)",
          WebkitBackdropFilter: "blur(8px)",
      ...
      >
        <span className="font-bold" style={{ fontSize: "9px", color: cfg.color }}>{cfg.icon}</span>
      </div>
      {/* Hover tooltip */}
      <div
        className="absolute left-1/2 -translate-x-1/2 bottom-full mb-1.5 opacity-0 group-hover/ep:opacity-100 transition-opacity duration-150 pointer-events-none whitespace-nowrap px-2 py-1 rounded-md text-[10px]"
...
/* ─── Config helper ─── */
function getConfig(type: string): { icon: string; color: string } {
  ...
  return configs[type] || configs.nav;
}
...
function EntryPointNodeComponent({ data }: NodeProps<EntryPointGroupData>) {
  ...
  // Compact display for other pages: icons in a row, labels on hover
  return (
    <>
      <div className="flex items-center justify-center gap-[4px]">
        {data.entryPoints.map((ep, i) => (
          <CompactBadge key={i} type={ep.type} label={ep.label} />
        ))}
      </div>
      <Handle type="source" position={Position.Bottom} className="!opacity-0 !w-0 !h-0" />
    </>
